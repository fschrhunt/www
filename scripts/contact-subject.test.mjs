...
test("subjects identify the topic and product without repeating the opening sentence", () => {
  assert.equal(suggestContactSubject("Hey Fischer, Diffuse crashes when I switch windows."), "Bug report for Diffuse");
...
test("personal and unfamiliar topics get a short fallback instead of a copied sentence", () => {
  ...
  assert.equal(suggestContactSubject("Reach me at diffuse@example.com or https://example.com/bug."), "A note for Fischer");
});
...
test("generated subjects stay between three and five words across supported topics", () => {
  for (const topic of ["a bug", "a suggestion", "a question", "feedback", "thanks", "a story"]) {
    for (const context of ["", " about Diffuse", " about the website"]) {
...
test("subjects use the product in the request rather than the first product mentioned", () => {
  assert.equal(suggestContactSubject("I love Diffuse. 𝑒 crashes on launch."), "Bug report for 𝑒");
  assert.equal(suggestContactSubject("I use Diffuse. Your contact form is broken."), "A website bug report");
  assert.equal(suggestContactSubject("Diffuse and 𝑒 both crash."), "A software bug report");
});

test("explicit requests outrank greetings, thanks, and incidental questions", () => {
  assert.equal(suggestContactSubject("How are you? Could you add keyboard shortcuts to Diffuse? Thanks!"), "Feature idea for Diffuse");
  assert.equal(suggestContactSubject("Thanks for Diffuse. We are hiring and would like you to join our team."), "A potential job opportunity");
  assert.equal(suggestContactSubject("This is not a bug report. Could you add themes to Diffuse?"), "Feature idea for Diffuse");
  assert.equal(suggestContactSubject("Diffuse is not broken. I have some feedback."), "Some feedback on Diffuse");
});
