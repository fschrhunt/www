...
let active=-1,visible=false,muted=false,unlocked=false,audio=null,offset=0,pitch=11,first=0,lastTick=0,last=179,guideX=0,guideTarget=0,guideVelocity=0,frame=0,previousTime=0,drag=null,anchorX=innerWidth/2,initialized=false;
...
function renderUsageRows(rows){details.replaceChildren(...rows.map(item=>{const row=document.createElement('div');row.className='row';const name=document.createElement('span');name.textContent=item.name;const price=document.createElement('span');price.className='price';price.textContent=item.value===null?'—':cost(item.value);row.append(ProviderIcons.create(item.modelId,item.creator),name,price);return row}))}
...
let loading=false,usageUrl=null;
async function loadUsage(){
 if(loading)return;loading=true;
 try{
  if(!usageUrl){const config=await fetch('/token-usage-assets/config.json');if(!config.ok)throw Error('Usage configuration unavailable');usageUrl=(await config.json()).url;if(!/^https:\/\/[a-z0-9]+\.public\.blob\.vercel-storage\.com\/usage\.json$/.test(usageUrl))throw Error('Invalid usage URL')}
  const response=await fetch(usageUrl,{signal:AbortSignal.timeout(10000)});
